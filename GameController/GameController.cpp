...
bool GameController::handleGlobalCommand(const std::string& command)
{
  ...
  else if(command == "set")
  {
    ...
    #ifdef DRIBBLE_GOAL
    SimulatedRobot::moveBall(Vector3f(0.f, 0.f, 50.f), true); //在这里变化球的初始位置；
    for(int i = 0; i < numOfRobots; ++i)
    {
    const Robot& r = robots[i];
    if(r.manuallyPlaced)
      r.simulatedRobot->moveRobot(Vector3f(500.f, 0, dropHeight),
                                  Vector3f(0, 0, 3.14f), true);
    }
    #endif
    #ifdef DRIBBLE_ROUND
    SimulatedRobot::moveBall(Vector3f(1800.f, 0.f, 50.f), true);
    for(int i = 0; i < numOfRobots; ++i)
    {
    const Robot& r = robots[i];
    if(r.manuallyPlaced)
      r.simulatedRobot->moveRobot(Vector3f(2200.f, 0, dropHeight),
                                  Vector3f(0, 0, 3.14f), true);
    #endif
    #ifdef HFO
...
void GameController::placeOffensivePlayers(int minRobot)
{
  static const Pose2f poses[2][numOfFieldPlayers] =
  {
    // 球员位置在这里改
    {
      Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 500, 0),
      Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 500, 0),
      Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 500, 0),
      Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 500, 0),

      // Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 1000, 0),
      // Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 1000, -1000),
      // Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 1000, -2000),
      // Pose2f(0, -fieldDimensions.xPosOpponentPenaltyMark - 1000, -3000),
    ...
    {
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 500, 0),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 500, 1000),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 500, 2000),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 500, 3000),
...
void GameController::placeDefensivePlayers(int minRobot)
{
  static const Pose2f poses[2][numOfFieldPlayers] =
  {
    ...
    {
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 1000, 0),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 1000, 0),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 1000, 0),
      Pose2f(-pi, fieldDimensions.xPosOwnPenaltyMark + 1000, 0),
...
void GameController::executePlacement()
{
  ...
  //   r.simulatedRobot->moveRobot(Vector3f(1000*i, 1000*i, dropHeight),
  //                                 Vector3f(0, 0, r.lastPose.rotation), true);
  }
}
      ...
      case STATE_PLAYING:
        switch(updateBall())
        {
          case goalByBlue:
            ...
            VERIFY(handleGlobalCommand("initial"));
            VERIFY(handleGlobalCommand("ready"));
            VERIFY(handleGlobalCommand("set"));
            break;
          case goalByRed:
          ...
          case notOut:
          #ifdef DRIBBLE_GOAL
            if (Time::getTimeSince(timeWhenStateBegan) >= 60000) { //运行1分钟后,还没进球，就给红方进一个球，以reset比赛；
                ++teamInfos[TEAM_RED].score;
                VERIFY(handleGlobalCommand("kickOffRed"));
                VERIFY(handleGlobalCommand("initial"));
                VERIFY(handleGlobalCommand("ready"));
...
GameController::BallOut GameController::updateBall()
{
  ...
  if(!fieldDimensions.isInsideField(ballInnerEdge))
  {
    ...
    else
    {
      ...
      // ballPos.x() = Pose2f(lastBallContactPose.rotation, ballPos).translation.x();
      // ballPos.y() = Pose2f(lastBallContactPose.rotation, ballPos).translation.y();
